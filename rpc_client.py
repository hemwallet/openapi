...
class FullNodeRpcClient:
    ...
    @classmethod
    async def create_by_proxy_url(cls, proxy_url, chia_root_path):
        self = cls()
        self.url = proxy_url
        self.session = aiohttp.ClientSession()
        root_path = Path(chia_root_path)
    ...
    async def get_coin_records_by_puzzle_hash(
        ...
        d = {"puzzle_hash": puzzle_hash, "include_spent_coins": include_spent_coins}
    ...
    async def get_coin_records_by_puzzle_hashes(
        ...
        d = {"puzzle_hashes": puzzle_hashes, "include_spent_coins": include_spent_coins}
        if start_height is not None:
            d["start_height"] = start_height
        if end_height is not None:
            d["end_height"] = end_height
    ...
    async def get_coin_record_by_name(self, coin_id: str):
        try:
            response = await self.fetch("get_coin_record_by_name", {"name": coin_id})
        except Exception:
            return None
    ...
    async def get_coin_records_by_names(
        ...
        names_hex = [name for name in names]
    ...
    async def whether_coins_spent(self, names: List[str]):
        coin_records = await self.get_coin_records_by_names(names)
        # print(type(coin_records),coin_records[0]['spent'])
        return [_['spent_block_index'] for _ in coin_records]

    async def get_puzzle_and_solution(self, coin_id: str, height: int):
        try:
            response = await self.fetch("get_puzzle_and_solution", {"coin_id": coin_id, "height": height})
            return response
